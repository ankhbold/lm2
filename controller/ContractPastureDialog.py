...
class ContractPastureDialog(QDialog, Ui_ContractPastureDialog, DatabaseHelper):
    ...
    def __init__(self, plugin, contract, navigator, attribute_update, parent=None):
        ...
        self.tab_index = self.pasture_value_tab.currentIndex()
        self.pasture_value_tab.currentChanged.connect(self.onChangetab)
    ...
    def __setup_validators(self):
        ...
        self.numbers_validator = QRegExpValidator(QRegExp("[0-9]{2}\\.[0-9]{3}"), None)
    ...
    def __set_up_twidget(self, table_widget):
        ...
        self.point_twidget.setAlternatingRowColors(True)
        self.point_twidget.setSelectionBehavior(QTableWidget.SelectRows)
        self.point_twidget.setSelectionMode(QTableWidget.SingleSelection)
        ...
        # self.point_twidget.horizontalHeader().setResizeMode(2, QHeaderView.Stretch)
        # self.point_twidget.horizontalHeader().setResizeMode(3, QHeaderView.Stretch)
    ...
    def __add_fee_row(self, row, contractor, parcel_id, base_fee_per_m2, subsidized_area, subsidized_fee_rate):
        ...
        item.setData(Qt.UserRole, contractor.person_ref.person_id)
        self.land_fee_twidget.setItem(row, CONTRACTOR_NAME, item)
        item = QTableWidgetItem(u'{0}'.format(contractor.person_ref.person_id))
        self.__lock_item(item)
    ...
    def __load_pasture_values(self):
        ...
        pasture_values = self.session.query(ClPastureValues).all()
        ...
        for pasture_value in pasture_values:
            item = QTableWidgetItem(pasture_value.description)
            item.setData(Qt.UserRole, pasture_value.code)
            count = self.pasture_values_twidget.rowCount()
            self.pasture_values_twidget.insertRow(count)
            self.pasture_values_twidget.setItem(count, 0, item)
            column = 1
            header = self.begin_year_sbox.value()
            for col in range(self.end_year_sbox.value() - self.begin_year_sbox.value()):
                p_value = 0
                value_count = self.session.query(PsPointPastureValue).filter(PsPointPastureValue.contract == self.contract.contract_no).\
                    filter(PsPointPastureValue.parcel == parcel_id).\
                    filter(PsPointPastureValue.pasture_value == pasture_value.code). \
                    filter(PsPointPastureValue.value_year == header).count()
                if value_count == 1:
                    value = self.session.query(PsPointPastureValue).filter(
                        PsPointPastureValue.contract == self.contract.contract_no). \
                        filter(PsPointPastureValue.parcel == parcel_id). \
                        filter(PsPointPastureValue.pasture_value == pasture_value.code).\
        ...
        # header = self.pasture_values_twidget.horizontalHeader()
        # header.setResizeMode(QHeaderView.Stretch)
    ...
    #         self.__load_monitoring_values()

    def __load_monitoring_values(self):
        ...
        if self.parcel_cbox.currentIndex() != -1:
            ...
            monitoring_points = self.session.query(CaPastureMonitoring) \
                .filter(CaPastureMonitoring.geometry.ST_Within(parcel.geometry)).all()
            ...
            for monitoring_point in monitoring_points:
                ...
                layer = LayerUtils.layer_by_data_source("s" + DatabaseUtils.current_working_soum_schema(), "ca_pasture_monitoring")
    ...
    @pyqtSlot()
    def on_pasture_value_save_button_clicked(self):
    ...
    def __save_pasture_values(self):
        ...
        for row in row_count:
            pasture_code =self.pasture_values_twidget.item(row, 0).data(Qt.UserRole)
            for column in column_count:
                if column+1 <= (self.pasture_values_twidget.columnCount() - 1):
                    ...
                    # print self.contract.contract_no +'--'+ parcel_id +'--'+ str(year_item) +'--'+ str(value_item) +'--'+ str(pasture_code)

                    value_count = self.session.query(PsPointPastureValue).filter(
                        PsPointPastureValue.contract == self.contract.contract_no). \
                        filter(PsPointPastureValue.parcel == parcel_id). \
                        filter(PsPointPastureValue.pasture_value == pasture_code). \
                        filter(PsPointPastureValue.value_year == year_item).count()
                    if value_count == 1:
                        pasture_value = self.session.query(PsPointPastureValue).filter(
                            PsPointPastureValue.contract == self.contract.contract_no). \
                            filter(PsPointPastureValue.parcel == parcel_id). \
                            filter(PsPointPastureValue.pasture_value == pasture_code). \
                        ...
                        pasture_value.contract = self.contract.contract_no
                        pasture_value.parcel = parcel_id
                        pasture_value.current_value = float(value_item)
    ...
    @pyqtSlot(int)
    def on_parcel_cbox_currentIndexChanged(self, index):

        parcel_id = self.parcel_cbox.itemData(self.parcel_cbox.currentIndex(), Qt.UserRole)
        app_no = self.application_this_contract_based_edit.text()
        parcel_pasture_count = self.session.query(CtApplicationParcelPasture).filter(CtApplicationParcelPasture.parcel == parcel_id).\
            filter(CtApplicationParcelPasture.application == app_no).count()
        pasture_label = ''

        if parcel_pasture_count > 0:
            parcel_pastures = self.session.query(CtApplicationParcelPasture).filter(
                CtApplicationParcelPasture.parcel == parcel_id). \
            ...
            for parcel_pasture in parcel_pastures:
                ...
                if pasture_label != '':
                    pasture_label = pasture_label + '-'
                pasture_label = pasture_label + pasture.description

            self.pasture_types_edit.setText(pasture_label)
    ...
    def on_new_point_button_clicked(self):
        ...
        point_count = self.session.query(CaPastureMonitoring) \
            .filter(CaPastureMonitoring.geometry.ST_Within(parcel.geometry)).count()
        point_id = None
        if point_count == 0:
            point_id = parcel_id + '001'
        else:
            cu_max_number = self.session.query(CaPastureMonitoring.point_id) \
                .filter(CaPastureMonitoring.geometry.ST_Within(parcel.geometry)) \
                .order_by(CaPastureMonitoring.point_id.desc()).first()
            cu_max_number = cu_max_number[0]
            if len(cu_max_number) == 15:
                point_end_id = cu_max_number[-3:]
                point_end_id = int(point_end_id) + 1
            else:
                point_end_id = '001'

            point_id = parcel_id + point_end_id
    ...
    def on_add_point_button_clicked(self):
        ...
        parcel_id = self.parcel_cbox.itemData(self.parcel_cbox.currentIndex(), Qt.UserRole)
        if not parcel_id:
            PluginUtils.show_message(self, self.tr("Parcel"), self.tr("Please choose parcel!!!"))
            return
        ...
        if is_coordinate:
            PluginUtils.show_message(self, self.tr("Coordinate"), self.tr("This point already registered!!!"))
            return
    ...
    def on_point_twidget_itemClicked(self, item):
        ...
        layer = LayerUtils.layer_by_data_source("s" + DatabaseUtils.current_working_soum_schema(),
    ...
    def on_point_zoom_button_clicked(self):
        ...
        if len(feature_ids) == 0:
            self.error_label.setText(self.tr("No point assigned"))
